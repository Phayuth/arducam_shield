...
sys.path.append(get_package_share_directory("arducam_shield") + "/libfiles/") # add folder path contain .so file
import time
import numpy as np
import cv2
import threading
import ArducamSDK
from arducam_shield import arducam_config_parser
...
class ArducamCamera(object):
    ...
    def openCamera(self, fname, index=0):
        self.isOpened, self.handle, self.cameraCfg, self.color_mode = Utils.camera_initFromFile(fname, index)
    ...
    def capture_thread(self):
        ...
        if ret != 0:
            self.running_ = False
            raise RuntimeError("Error beginning capture, Error : {}".format(Utils.GetErrorString(ret)))

        print("Capture began, Error : {}".format(Utils.GetErrorString(ret)))

        while self.running_:
            ret = ArducamSDK.Py_ArduCam_captureImage(self.handle)
            if ret > 255:
                print("Error capture image, Error : {}".format(Utils.GetErrorString(ret)))
    ...
    def getCamInformation(self):
        ...
        cpldVersion = "V{:d}.{:d}\t20{:0>2d}/{:0>2d}/{:0>2d}".format(self.version >> 4, self.version & 0x0F, self.year, self.mouth, self.day)
        return cpldVersion

    def getMipiDataInfo(self):
        mipiData = {"mipiDataID": "", "mipiDataRow": "", "mipiDataCol": "", "mipiDataClk": "", "mipiWordCount": "", "mFramerateValue": ""}
        ...
        if (cpld_version == 0x30 and date >= (21 * 1000 + 3 * 100 + 1)) or (cpld_version == 0x20 and date >= (21 * 1000 + 9 * 100 + 6)):
...
class Utils:
    ...
    def GetErrorString(ErrorCode):
        return Utils.ErrorCode_Map[ErrorCode]
    ...
    def camera_initFromFile(fileName, index):
        ...
        if ret == 0:
            ...
            for i in range(configs_length):
                ...
                if type & 0xFFFF == arducam_config_parser.CONFIG_TYPE_REG:
                    ...
                    Utils.configBoard(handle, configs[i])
        ...
        print("open fail, Error : {}".format(Utils.GetErrorString(ret)))
...
class ImageConverter:
    ...
    def convert_color(image, color_mode):
        if color_mode == 0:
            image = cv2.cvtColor(image, ImageConverter.COLOR_BayerRG2BGR)
        if color_mode == 1:
            image = cv2.cvtColor(image, ImageConverter.COLOR_BayerGR2BGR)
        if color_mode == 2:
            image = cv2.cvtColor(image, ImageConverter.COLOR_BayerGB2BGR)
        if color_mode == 3:
            image = cv2.cvtColor(image, ImageConverter.COLOR_BayerBG2BGR)
    ...
    def convert_image(data, cfg, color_mode):
        ...
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_JPG:
            image = ImageConverter.JPGToMat(data, datasize)
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_YUV:
            image = ImageConverter.YUVToMat(data, Width, Height, color_mode)
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_RGB:
            image = ImageConverter.RGB565ToMat(data, Width, Height)
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_MON:
            if cfg["u8PixelBytes"] == 2:
                image = ImageConverter.dBytesToMat(data, bitWidth, Width, Height)
            else:
        ...
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_RAW:
            if cfg["u8PixelBytes"] == 2:
                ...
                image = np.frombuffer(data, np.uint8).reshape(Height, Width, 1)
            image = ImageConverter.convert_color(image, color_mode)
        if emImageFmtMode == ArducamSDK.FORMAT_MODE_RAW_D:
            image = ImageConverter.separationImage(data, Width, Height)
            image = ImageConverter.convert_color(image, color_mode)
